...
public class StudentView {
	 ...
	 * 학생 추가 화면*
	 * 
	 * 학번, 이름, 성별을 입력받아 Student 객체로 만들어서 StudentService로 전달! -> 추가 성공시 true / 실패시
	 * false 반환 받아 결과 출력.
	 */
	private void addStudent() {
		...
		// StudentService 객체의 addStduent()호출 후
		// 결과 반환 받기
	...
	/**
	 * 
	 * StudentService에 있는 students 객체 배열을 얻어와 모든 학생정보를 출력
	 */
...
//		/향상된 for문
		for (StudentDTO std : students) {
			...
			// null인경우 == 저장된 학생이 없을 떄
			// -> NullPointerException 방지
			// (참조변수가 null을 저장하고 있는데 사용하려고 할떄 )
			if (std == null)
				return;
	...
	/*
	 * 인덱스를 입력받아 service에 전달하여 해당 인덱스의 학생 객체 하나 (StudentDTO)를 반환 받아 출려
	 * 
	 * 
	 * 
	 */
	private void selectIndex() {
		...
		if (std == null) {
			System.out.println("*** 해당 인덱스에 학생 개체가 존재하지 않습니다.***");
			return;
		}
	...
	/**
	 * 이름을 입력 받아 service의 students 배열에서 같은 이름을 가진 학생 개쳋를 얻어와 출력 -> 이름검색
	 */
