...
public class StudentService {
	...
	public StudentService() {
		...
		// 학생 객체 배열 (student)에 샘플 학생 3명 추가
		...
		// student[3] / [4] == null 아직 아무것도 참조하지 않고있다

		// 0~ 100 사이 난수 발생
		// 1) Math.flor(Math.random()* 101) == 0 ~ 100
	...
	/**
	 * 학생 객체 배열 students 증 비어잇는 인덱스에 전달받은 학생 객체 (std) 추가
	 * 
	 * @param std : 입력받은 값을 저장한 학생 객체 주소
	 * @return : 추가 성공시 true / false : 추가 실패 - 실패 조건 : students 배열에 꽉 차있는데 추가 학생이
	 *         전달된 경우
	 * 
	 */
...
//	}
	...
	public StudentDTO selectIndex(int index) {
		...
		// 전달 받은 index범위가 students의 인덱스 범위 밖인 경우
	...
	/**
	 * 전달 받은 index가 students 범위 내인지 + 전상 범위라면 해당 index가 학생 객체를 참조하는 확인
	 * 
	 * @param index
	 * @return 1 또는 2 또는 (정상)
	 */
	...
	public void updateScores(int index, StudentDTO other) {
		...
		// 객체 배열 : 객체 참조형 변수들 묶음으로 다룸
		
		
		//students[index] 번쨰에 저장된 주소 얕은 복사
...
//		return;// 안써도 컴파일러가 자동으로 추가
	...
	public String selectMaxMin() {
		...
		for(StudentDTO std : students) {
			...
			if(avg > maxAvg) { // 기존 최대값 보다 현재 평균이 클 떄
				maxAvg = avg;
				maxName = std.getName();
			}
			if(avg < minAvg) { // 기존 최소값 보다 현재 평균이 클 떄
				minAvg = avg;
				minName = std.getName();
			}
		}
