...
public class LoopEx2 {
	...
	public void method3() {
		...
		for(int i = 1 ; i <= col; i++) { //행 
			
		if(direction == 1) {
			for(int y = 1 ; y <= col ; y++) {//정방향	
				System.out.println(y);
			}
			
		}else {
			for(int y = col ; y >= 1 ; y--) {
				System.out.println(y); // 역방향.
			}
		}
		...
		for (int x = 1; x <= row; x++) {
			for (int y = start ;/*공백 무한 반복*/ ; y += direction) {
	...
	public void method4() {
		for (int row = 0 ; row < 3 ; row++) { // 0~2 행(1번칸) 0,1,2(3개 (줄))
			
			for(int col = 0 ; col < 3 ; col ++) { // 0 ~2 열 (2번칸)
	...
	public void method6() {
		for (int row = 0 ; row < 3 ; row++) { // 0~2 행(1번칸) 0,1,2(3개 (줄))
			...
			for(int col = 0 ; col < 3 - row; col ++) { // 0 ~2 열 (2번칸)
