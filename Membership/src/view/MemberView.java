...
public class MemberView {
	...
	public MemberView() {
		...
		//  초기화 후 inputStreamReader 입력 스트림 생성.
		}catch (Exception e) {
	...
	public void mainMenu() {
		...
		do {
			//do 무조건 한번 실행하는 문구.
			try {
				//try로 case문에 어디가 문제가 있는지 확인한다.
				//chtch부분으로 어디에서 오류가 생겼는지 받을수있다.
				...
				//NumberFormatException 숫자형식으로 입력하지 않을시 오류 발생.
			} catch(NumberFormatException e) {
				System.out.println("\n### 숫자만 입력 해주세요 ###\n");
				input = -1; // 첫 반복에서 종료되지 않게 값 변경
			// input = -1 첫 반복에서 숫자오류가 난다면 종료가아닌 다시 실행.
			} catch(IOException e) {
				System.out.println("\n### 입출력 관련 예외 발생 ###\n");
			...
			//catch 그 외 에 오류 발생.
				...
				e.printStackTrace();
				// catch로 Excetion(나머지 오류 전부다) 오류 체크.
			}
			
			
		}while(input != 0);
		//while input 0아닐시 화일 반복문 끝.
		//case -> 화면에 나올 문구 추가 
	}
